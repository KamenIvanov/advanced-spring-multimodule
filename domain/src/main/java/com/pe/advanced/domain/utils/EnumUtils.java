package com.pe.advanced.domain.utils;
...
public class EnumUtils {
    ...
    public static <T extends Enum<T>> T enumForName(Class<T> enumClass, String enumElementName) {
        ...
        Class<T> c = (Class<T>) (isSpecializedEnumConstant(enumClass) ? enumClass.getSuperclass() : enumClass);
        return Enum.valueOf(c, enumElementName);
    }

    /**
     * Tells if a class is an enumeration or specialized enum constant. Specialized enum constants are implemented by the
     * JVM as enum inner classes, extending the enum. However, their ENUM flag bit is not set, and {@link Class#isEnum()}
     * returns false. Clients of this method are convertors that require a clear indication if an object can be
    ...
    /**
     * Tells if the given class is a specialized enum constant. JVM represents it as an inner class of the enum, and
     * Class.isEnum() returns false
     *
     * @param clazz the given class
     * @return true if it's enum constant, false otherwise
     */
    public static boolean isSpecializedEnumConstant(Class<?> clazz) {
